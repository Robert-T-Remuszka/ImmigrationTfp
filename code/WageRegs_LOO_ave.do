...
loc graphs "Wage00Response_Iv1990_LOO_ave Wage01Response_Iv1990_LOO_ave Wage00Response_Iv1990_LOO_ave_F Wage01Response_Iv1990_LOO_ave_F"
...
* -----------------------------------------------------------
* Robustness: LOO average growth rate (equal-weight)
* -----------------------------------------------------------

quietly ds BodiesSupplied_*, has(type numeric)
local bodyvars `r(varlist)'

* Per-state stock growth by group and year
foreach v of local bodyvars {
    local region = subinstr("`v'","BodiesSupplied_","",1)

    * state-level stock growth: gstock_{m,s,t}
    bys statefip (year): gen double gstock_`region' = ///
        (`v' - `v'[_n-1]) / `v'[_n-1]
    bys statefip (year): replace gstock_`region' = . if `v'[_n-1]==0 | missing(`v'[_n-1])

    * within-year simple average across states
    bys year: egen double gstock_meanyr_`region' = mean(gstock_`region')

    * LOO equal-weight average with fixed 1/51:
    gen double gstockloo_eq_`region' = (52*gstock_meanyr_`region' - gstock_`region')/51
}

* Build Bartik with fixed 1990 shares
quietly ds gstockloo_eq_*
foreach v in `r(varlist)' {
    local region = subinstr("`v'","gstockloo_eq_","",1)
...
label var Bartik_1990_LOO "LOO Bartik using equal-weight (1/51) stock-growth shocks, 1990 shares"
...
frame Estimates {
    ...
    gen SeForeignIv1990 = .
    gen FForeignIv1990 = .
	gen BetaForeignIv1990_LOO = .
    gen SeForeignIv1990_LOO = .
    gen FForeignIv1990_LOO = .


    gen BetaDomesticIv1990 = .
    gen SeDomesticIv1990 = .
    gen FDomesticIv1990 = .
	gen BetaDomesticIv1990_LOO = .
    gen SeDomesticIv1990_LOO = .
    gen FDomesticIv1990_LOO = .
...
forvalues h = -3/9 {
    ...
    if `h' != -1 {
		...
		* LOO IV regression
        qui ivreghdfe W01g_`horizon' (fg = Bartik_1990_LOO) [pw = emp] if `samp', absorb(state year) vce(robust)
        ...
        * Record the results in the Estimates frame
        frame Estimates {
			...
			* Store LOO results
            replace BetaForeignIv1990_LOO = _b[fg] if _n == _N
            replace SeForeignIv1990_LOO = _se[fg] if _n == _N
            replace FForeignIv1990_LOO = `widstat_loo' if _n == _N
        }
		...
		* LOO IV regression
        qui ivreghdfe W00g_`horizon' (fg = Bartik_1990_LOO) [pw = emp] if `samp', absorb(state year) vce(robust)
        local widstat_loo = `e(widstat)'
        ...
        frame Estimates {
			...
			* Store LOO results
            replace BetaDomesticIv1990_LOO = _b[fg] if _n == _N
            replace SeDomesticIv1990_LOO = _se[fg] if _n == _N
            replace FDomesticIv1990_LOO = `widstat_loo' if _n == _N
        }
    ...
    * Fill in the horizon -1 results
        ...
        frame Estimates {
			...
			* Store LOO results
            replace BetaForeignIv1990_LOO = 0 if _n == _N
            replace SeForeignIv1990_LOO = . if _n == _N
            replace FForeignIv1990_LOO = . if _n == _N
            *Domestic
            replace BetaDomesticIv1990 = 0 if _n == _N
            replace SeDomesticIv1990 = . if _n == _N
            replace FDomesticIv1990 = . if _n == _N
			* Store LOO results
            replace BetaDomesticIv1990_LOO = 0 if _n == _N
            replace SeDomesticIv1990_LOO = . if _n == _N
            replace FDomesticIv1990_LOO = . if _n == _N
        }
    }
...
frame Estimates {
    ...
    * Confidence intervals - 95%
    gen BetaForeignIv1990_LOO_upper = BetaForeignIv1990_LOO + 1.96 * SeForeignIv1990_LOO
    gen BetaForeignIv1990_LOO_lower = BetaForeignIv1990_LOO - 1.96 * SeForeignIv1990_LOO

    gen BetaDomesticIv1990_LOO_upper = BetaDomesticIv1990_LOO + 1.96 * SeDomesticIv1990_LOO
    gen BetaDomesticIv1990_LOO_lower = BetaDomesticIv1990_LOO - 1.96 * SeDomesticIv1990_LOO
	
	tw connected BetaForeignIv1990_LOO h if inrange(h,-3, 9), ms(oh) mc("0 147 245") xlab(-3(1)9, nogrid) sort || rcap BetaForeignIv1990_LOO_upper BetaForeignIv1990_LOO_lower h, lcolor("0 147 245") ylab(, nogrid) ///
    ytitle("{&epsilon}{subscript:F}") xtitle("Horizon") legend(off) yline(0, lc(black%50) lp(solid)) name(Wage01Response_Iv1990_LOO_ave)

    tw connected BetaDomesticIv1990_LOO h if inrange(h,-3, 9), ms(oh) mc("0 147 245") xlab(-3(1)9, nogrid) sort || rcap BetaDomesticIv1990_LOO_upper BetaDomesticIv1990_LOO_lower h, lcolor("0 147 245") ylab(, nogrid) ///
    ytitle("{&epsilon}{subscript:D}") xtitle("Horizon") legend(off) yline(0, lc(black%50) lp(solid)) name(Wage00Response_Iv1990_LOO_ave)
}

* F Stats of IRF for pre-period
frame Estimates {

    graph bar FForeignIv1990_LOO if h > -1, over(h) bar(1, color("0 147 245") fcolor("0 147 245")) ylab(0(20)180, nogrid labsize(small)) ///
    yline(10,lc(black%70) lp(dash)) legend(off) b1title("Horizon") ytitle("First Stage F") name(Wage01Response_Iv1990_LOO_ave_F)

    graph bar FDomesticIv1990_LOO if h > -1, over(h) bar(1, color("0 147 245") fcolor("0 147 245")) ylab(0(20)180, nogrid labsize(small)) ///
    yline(10,lc(black%70) lp(dash)) legend(off) b1title("Horizon") ytitle("First Stage F") name(Wage00Response_Iv1990_LOO_ave_F)
