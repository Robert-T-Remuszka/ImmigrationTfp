...
foreach lag of numlist 1/`numlags' {
    ...
    eststo L`lag': qui ivreg2 fg Bartik_1990 L(1/`lag').D.Z i.year i.state L(1/4).fg [pw = emp] if `samp', dkraay(9) partial(i.year i.state)

    loc models "`models' L`lag'"

}    

esttab `models' using "${Tables}/Lag_order_Z.tex", replace booktabs label se nocons drop(*.fg) ///
