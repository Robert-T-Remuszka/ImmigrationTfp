...
loc graphs "LResponse_Iv1990_LOO_ave LResponse_Iv1990_LOO_ave_F"
...
forvalues h = -3/9 {
    ...
    if `h' != -1 {
		...
		* LOO IV regression
        qui ivreghdfe Lg_`horizon' (fg = Bartik_1990_LOO) [pw = emp] if `samp', absorb(state year) vce(robust)
...
frame Estimates {
    ...
    * Confidence intervals - 95% for LOO
    ...
    ytitle("{&eta}{subscript:L}") xtitle("Horizon") legend(off) yline(0, lc(black%50) lp(solid)) name(LResponse_Iv1990_LOO_ave)
}
...
frame Estimates {
    ...
    yline(10,lc(black%70) lp(dash)) legend(off) b1title("Horizon") ytitle("First Stage F") name(LResponse_Iv1990_LOO_ave_F)
